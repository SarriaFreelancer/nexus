...
import { Bell, Check, AlertTriangle, Clock, AlertCircle, ArrowRight, ExternalLink } from "lucide-react";
      ...
      <div className="flex items-center justify-between">
        ...
        {unreadCount > 0 && (
          <button 
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100 transition-colors cursor-pointer shadow-sm"
          >
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3">
                <div className="mt-0.5 shrink-0">
                  {isOverdue ? (
                    ...
                    <span className="h-2.5 w-2.5 rounded-full bg-indigo-500 animate-pulse block mt-1" />
                  ) : (
                ...
                <div className="space-y-1">
                  <h4 className="font-bold text-slate-900 dark:text-slate-100 text-xs flex items-center gap-2">
                    <span>{n.title}</span>
                    {!n.isRead && (
                      <span className="px-2 py-0.5 rounded-md bg-indigo-500 text-white text-[9px] font-black uppercase">
                        NUEVA
                      </span>
                    )}
                  </h4>
              ...
              <div className="flex items-center gap-2 shrink-0 ml-4">
                <button
                  type="button"
                  className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-[11px] transition-all shadow-sm group-hover:scale-105"
                >
                  <span>Ir a Tarea</span>
