...
export default function RoadmapsPage() {
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto pb-10">
      ...
      <div className="p-4 rounded-2xl bg-[#0f1424] border border-slate-800/80 overflow-x-auto">
        <div className="flex items-center min-w-[1100px] gap-2">
          {lifecycleStages.map((stage, idx) => (
            <React.Fragment key={idx}>
              <div className="flex-1 p-3 rounded-xl bg-slate-900/80 border border-slate-800 text-center space-y-1">
      ...
      <div className="space-y-4">
        <h3 className="text-sm font-bold text-slate-100">Estado del Pipeline por Proyecto</h3>

        <div className="space-y-4">
          {mockProjects.map((proj) => (
            <div
              key={proj.id}
              className="p-5 rounded-2xl bg-[#0f1424] border border-slate-800/80 space-y-4 hover:border-slate-700 transition-all"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-slate-400">Versión actual: v{proj.currentVersion}</span>
                  </div>
                </div>
